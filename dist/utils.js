...
const { processVariables, recursiveSubstitute } = TemplateEngine;
 ...
 */
export function suckOutVariationsOnly(reporters) {
    const variationsOut = {};
 ...
 */
export function suckOutEditions(reporters) {
    const editionsOut = {};
 ...
 * In other words, this lets you go from an edition match to its parent key.
 */
export function suckOutFormats(reporters) {
    const formatsOut = {};
    for (const [_reporterKey, dataList] of Object.entries(reporters)) {
        // For each reporter key...
        for (const data of dataList) {
 ...
 */
export function namesToAbbreviations(reporters) {
    const names = {};
    for (const [_reporterKey, dataList] of Object.entries(reporters)) {
        for (const data of dataList) {
